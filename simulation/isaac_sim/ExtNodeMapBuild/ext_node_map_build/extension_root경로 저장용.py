...
ROUTE_ACTION_NAME = "/visualize_route"
...
class ExtNodeMapBuild(omni.ext.IExt):
    ...
    def on_startup(self, ext_id: str) -> None:
        ...
        self._ros_node = None
        self._service = None
        self._route_action_server = None
        ...
        # NodeMap Service와 Planned Route Action Server를 생성한다.
    ...
    def _setup_ros_interfaces(self) -> None:
        ...
        self._route_action_server = ActionServer(
            self._ros_node,
            VisualizeRoute,
            ROUTE_ACTION_NAME,
        ...
        carb.log_info(
            f"[ExtNodeMapBuild] ROS endpoints ready: "
            f"{SERVICE_NAME}, {ROUTE_ACTION_NAME}"
        )
    ...
    def _execute_visualize_route(
        self,
        goal_handle,
    ) -> VisualizeRoute.Result:
        """AMR이 전달한 계획 경로를 검증하고 Runtime에 저장한다."""
        ...
        route_size = len(request.node_ids)
        coordinate_sizes = (
            len(request.node_x),
            len(request.node_y),
            len(request.node_z),
        ...
        if route_size == 0:
            return self._abort_route(
                goal_handle,
                result,
                "Route node_ids is empty.",
            )
        if any(size not in (0, route_size) for size in coordinate_sizes):
            return self._abort_route(
                goal_handle,
                result,
                "Route XYZ length is invalid.",
            )
        if len(set(coordinate_sizes)) != 1:
            return self._abort_route(
                goal_handle,
                result,
                "Route XYZ arrays differ.",
            )

        if request.node_map_revision != self._revision:
            return self._abort_route(
                goal_handle,
                result,
                f"NodeMap revision mismatch: "
                f"expected={self._revision}, received={request.node_map_revision}",
            )

        unknown_node_ids = [
            node_id for node_id in request.node_ids if node_id not in self._nodes
        ]
        if unknown_node_ids:
            return self._abort_route(
                goal_handle,
                result,
                f"Unknown Node IDs: {unknown_node_ids}",
            )

        edge_pairs = set()
        for edge in self._edges:
            edge_pairs.add((edge["start"], edge["end"]))
            if edge["bidirectional"]:
                edge_pairs.add((edge["end"], edge["start"]))

        for start, end in zip(request.node_ids, request.node_ids[1:]):
            if (start, end) not in edge_pairs:
                return self._abort_route(
                    goal_handle,
                    result,
                    f"Edge not found: {start} -> {end}",
                )

        points = (
            list(zip(request.node_x, request.node_y, request.node_z))
            if coordinate_sizes[0]
            else []
        )
        self._planned_routes[request.amr_id] = {
            "task_id": request.task_id,
            "revision": int(request.node_map_revision),
            "node_ids": list(request.node_ids),
            "points": points,
        }

        route_names = [
            self._nodes[node_id]["name"] for node_id in request.node_ids
        ]
        feedback.status = "VISUALIZED"
        goal_handle.publish_feedback(feedback)
        goal_handle.succeed()
        result.success = True
        result.message = (
            f"Stored route for {request.amr_id}, task={request.task_id}, "
            f"Nodes={route_size}"
        )
        carb.log_info(
            f"[ExtNodeMapBuild] Planned Route received: "
            f"AMR={request.amr_id}, Task={request.task_id}, "
            f"Route={' -> '.join(route_names)}"
        )
        return result

    @staticmethod
    def _abort_route(goal_handle, result, message):
        goal_handle.abort()
    ...
    def on_shutdown(self) -> None:
        ...
        if self._ros_node is not None:
            ...
            if self._route_action_server is not None:
                self._route_action_server.destroy()
